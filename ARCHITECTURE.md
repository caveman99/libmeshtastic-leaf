...
They meet at the implicit acknowledgement. A packet of this node's heard being
relayed is a sighting of sender and id where the sender is this node, so the
receive path detects it and stops the pending retransmission. The check uses the header
...
A test pins the frequencies and slots for EU_868, EU_866, EU_N_868, JP and US
to what firmware 2.8.0 computes.
...
Two airtime windows. This node's transmissions over a rolling hour give the
duty cycle. Everything heard plus
...
seconds, so staging and returning is the only workable shape for `send*()`.
...
Firmware 2.8 is on the air with a parameter set of its own, and a node that
differs on any of it is deaf to the mesh:

| Parameter        | Value                                              |
| ---------------- | -------------------------------------------------- |
| Sync word        | `0x2B`, neither the public nor the private default |
| Preamble         | 16 symbols, 12 above 2 GHz, against a default of 8 |
| CRC              | on, set by the sketch                              |
| Header           | explicit                                           |
| Spreading factor | from the preset                                    |
| Bandwidth        | from the preset                                    |
| Coding rate      | from the preset                                    |
...
There is no authentication. Anyone holding the PSK can forge any packet on the
channel, including an acknowledgement, and nothing detects a modified
ciphertext. Only PKI traffic is authenticated, by the AES-CCM tag. This is the
firmware's design and interoperating requires matching it.
...
The extra nonce sits at bytes 4 to 7, over the packet id's unused high half,
not at the end. Both ciphers build their nonce through one function, and a
test pins the layout.

Sending a direct message needs the recipient's public key alongside this
node's private key. The recipient needs this node's public key in return,
which it takes from the signed NodeInfo, so a node that has never announced
itself cannot be replied to.

## Randomness

Packet ids, the PKI extra nonce and key generation all come from `CryptRNG`,
the CSPRNG in the Crypto library, which draws on the platform TRNG where there
is one. On ESP32 that TRNG is `esp_random()`. Nothing calls `randomSeed()`: on
the ESP32 core that switches the Arduino `random()` over to a weaker PRNG.

`begin()` stirs sixteen bytes of radio noise into the pool. A keypair
generated before `begin()` rests on the platform TRNG alone. On a platform the
Crypto library has no TRNG for, stir entropy in from the sketch through
`stirEntropy()` in `MeshEntropy.h` first.

The packet id is a ten bit counter seeded from the CSPRNG at boot, with twenty
two random bits above it. A repeated id repeats a cipher nonce.

## Identity

Since firmware 2.8 a node number is the CRC-32 of the node's 32 byte public
key, not anything taken from the hardware. Key generation and storage belong to
the application; handing the public key in is what sets the node number, and
the copy lives in RAM only.

Generating a key belongs here even though storing one does not, because the
mesh has rules a key must satisfy and they are easy to miss: no weak Curve25519
point, nothing on the published low entropy list that the firmware regenerates
on, and a CRC-32 that is neither reserved nor the broadcast address. The
generator retries until it has one and reports failure rather than handing back
a key the mesh will not accept.

There is therefore no node number until a key arrives. The library reports that
rather than inventing one: `getNodeNum()` gives 0 and a send is refused with
`NO_IDENTITY`, since a packet whose sender is zero is dropped by every
receiver. The MAC derived numbering that predated 2.8 is gone, along with the
per platform code that read the hardware identifier.

A node announces itself with a NodeInfo carrying that key, the names, and
`PRIVATE_HW` as the hardware model, because this is a library and not a device.

That announcement has to be XEdDSA signed. A receiver seeing this node for the
first time has no key for it, so it verifies the signature against the key
inside the message and drops the packet if it does not check out. An unsigned
NodeInfo is therefore never learned, and the node stays unknown however many
messages it sends. The signature covers `[from | id | portnum | payload]`, all
little endian, and rides in `Data.xeddsa_signature`.

Only POSITION, TELEMETRY, WAYPOINT and NODEINFO are signable. Text is not, so
it is never dropped for lacking a signature, which keeps the 64 byte signature
out of the payload budget for ordinary messages. Every packet on a signable
portnum is signed once a private key is set.

The firmware announces every three hours and refuses to go under an hour, so
this does the same. It is the lowest priority transmission, behind
