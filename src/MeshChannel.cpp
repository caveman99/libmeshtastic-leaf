...
MeshChannel::MeshChannel() : hash_(0) {
...
void MeshChannel::expandPSK(const uint8_t* psk, size_t pskLen, CryptoKey& outKey) {
    // Clear the key
    ...
    size_t copyLen = (pskLen <= sizeof(outKey.bytes)) ? pskLen : sizeof(outKey.bytes);
...
bool MeshChannel::setChannel(const uint8_t* psk, size_t pskLen, const char* name) {
    // Store channel name
    memset(name_, 0, sizeof(name_));
    if (name && name[0]) {
