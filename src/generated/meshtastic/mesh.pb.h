...
to identify this board */
  ...
  /* WIO Tracker WM1110 family from Seeed Studio. Includes wio-1110-tracker and
     wio-1110-sdk */
...
--------------------------------------------------------------------------- */
  ...
  /* M5 esp32 based MCU modules with enclosure, TFT and LORA Shields. All
     Variants (Basic, Core, Fire, Core2, CoreS3, Paper) https://m5stack.com/ */
...
Lora module can be swapped out for a Heltec RA-62 which is "almost" pin
compatible with one cut and one jumper Meshtastic works */
...
Older "V1.0" Variant */
  ...
  /* unPhone with ESP32-S3, TFT touchscreen,  LSM6DS3TR-C accelerometer and
     gyroscope */
...
ESP32-D0WDQ6 With SX1276/SKY66122, SSD1306 OLED and No GPS */
  ...
  /* Heltec Capsule Sensor V3 with ESP32-S3 CPU, Portable LoRa device that can
     replace GNSS modules or sensors */
  ...
  /* Heltec Mesh Node T114 board with nRF52840 CPU, and a 1.14 inch TFT display,
Ultimate low-power design, specifically adapted for the Meshtatic project */
  meshtastic_HardwareModel_HELTEC_MESH_NODE_T114 = 69,
  /* Sensecap Indicator from Seeed Studio. ESP32-S3 device with TFT and RP2040
     coprocessor */
  meshtastic_HardwareModel_SENSECAP_INDICATOR = 70,
  /* Seeed studio T1000-E tracker card. NRF52840 w/ LR1110 radio, GPS, button,
     buzzer, and sensors. */
  meshtastic_HardwareModel_TRACKER_T1000_E = 71,
  /* RAK3172 STM32WLE5 Module
     (https://store.rakwireless.com/products/wisduo-lpwan-module-rak3172) */
  meshtastic_HardwareModel_RAK3172 = 72,
  /* Seeed Studio Wio-E5 (either mini or Dev kit) using STM32WL chip. */
  meshtastic_HardwareModel_WIO_E5 = 73,
  /* RadioMaster 900 Bandit,
https://www.radiomasterrc.com/products/bandit-expresslrs-rf-module SSD1306 OLED
and No GPS */
  meshtastic_HardwareModel_RADIOMASTER_900_BANDIT = 74,
  /* Minewsemi ME25LS01 (ME25LE01_V1.0). NRF52840 w/ LR1110 radio, buttons and
     leds and pins. */
...
^^^ short A0 to switch to I2C address 0x3C */
  ...
  /* Nordic nRF52840+Semtech SX1262 LoRa BLE Combo Module. nRF52840+SX1262
     MS24SF1 */
...
https://github.com/valzzu/Mesh-Tab */
  ...
  /* MeshLink board developed by LoraItalia. NRF52840, eByte E22900M22S (Will
also come with other frequencies), 25w MPPT solar charger (5v,12v,18v
selectable), support for gps, buzzer, oled or e-ink display, 10 gpios, hardware
watchdog https://www.loraitalia.it */
...
https://www.elecrow.com/wiki/ThinkNode-M2_Transceiver_Device(Meshtastic)_Power_By_NRF52840.html
(this actually uses ESP32-S3) */
  ...
  /* RAKwireless WisBlock Core RAK3312
  ...
  /* MeshSolar is an integrated power management and communication solution
designed for outdoor low-power devices. https://heltec.org/project/meshsolar/ */
...
didn't reply with an interrupt. */
  ...
  /* We detected that the main CPU voltage dropped below the minimum acceptable
     value */
...
we can ask you to collect some information to allow fixing this bug */
  ...
  /* Corruption was detected on the flash filesystem but we were able to repair
things. If you see this failure in the field please post in the forum because we
are interested in seeing if this is occurring in the field. */
  meshtastic_CriticalErrorCode_FLASH_CORRUPTION_RECOVERABLE = 12,
  /* Corruption was detected on the flash filesystem but we were unable to
repair things. NOTE: Your node will probably need to be reconfigured the next
time it reboots (it will lose the region code etc...) If you see this failure in
the field please post in the forum because we are interested in seeing if this
is occurring in the field. */
  meshtastic_CriticalErrorCode_FLASH_CORRUPTION_UNRECOVERABLE = 13
} meshtastic_CriticalErrorCode;

/* Enum to indicate to clients whether this firmware is a special firmware
 build, like an event. The first 16 values are reserved for non-event special
 firmwares, like the Smart Citizen use case. */
...
typedef enum _meshtastic_ExcludedModules {
  ...
  /* Bluetooth config (not technically a module, but used to indicate bluetooth
     capabilities) */
  meshtastic_ExcludedModules_BLUETOOTH_CONFIG = 8192,
  /* Network config (not technically a module, but used to indicate network
     capabilities) */
...
required bound channel) */
  ...
  /* The client specified a PKI transport, but the node was unable to send the
     packet using PKI (and did not send the message at all) */
  ...
  /* Admin packet sent using PKC, but not from a public key on the admin key
     list */
  meshtastic_Routing_Error_ADMIN_PUBLIC_KEY_UNAUTHORIZED = 37,
  /* Airtime fairness rate limit exceeded for a packet
This typically enforced per portnum and is used to prevent a single node from
...
This is different from PKI_UNKNOWN_PUBKEY which indicates a failure upon
receiving a packet */
...
assume it is important and use a slightly higher priority */
  ...
  /* If priority is unset but the packet is a response to a request, we want it
to get there relatively quickly. Furthermore, responses stop relaying packets
directed to a node early. */
  meshtastic_MeshPacket_Priority_RESPONSE = 80,
  /* Higher priority for specific message types (portnums) to distinguish
     between other reliable packets. */
  meshtastic_MeshPacket_Priority_HIGH = 100,
  /* Higher priority alert message used for critical alerts which take priority
     over other reliable packets. */
...
This is sent out to other nodes on the mesh to allow them to compute a shared
secret key. */
...
typedef struct _meshtastic_RouteDiscovery {
  ...
  /* The list of nodenums the packet has visited on the way back from the
   * destination. */
  pb_size_t route_back_count;
  uint32_t route_back[8];
  /* The list of SNRs (in dB, scaled by 4) in the route back from the
   * destination. */
...
typedef struct _meshtastic_KeyVerification {
  ...
  uint64_t nonce;
  /* The final authoritative hash, only to be sent by NodeA at the end of the
   * handshake */
...
/* This message will be proxied over the PhoneAPI for the client to deliver to
 * the MQTT server */
...
If the value is 4,294,967,295 (maximum value of an unsigned 32bit integer), this
indicates that the packet was not destined for a specific node, but for a
channel as indicated by the value of `channel` below. If the value is another,
this indicates that the packet was destined for a specific node (i.e. a kind of
"Direct Message" to this node) and not broadcast on a channel. */
  uint32_t to;
...
Used to collect statistics on current link quality. */
  float rx_snr;
  /* If unset treated as zero (no forwarding, send to direct neighbor nodes
only) if 1, allow hopping through one node, etc... For our usecase real world
topologies probably have a max of about 3. This field is normally placed into a
few of bits in the header. */
  uint8_t hop_limit;
...
See MeshPacket.Priority description for more details. */
  ...
  /* Describes whether this packet passed via MQTT somewhere along the path it
   * currently took. */
  bool via_mqtt;
  /* Hop limit with which the original packet started. Sent via LoRa using three
bits in the unencrypted header. When receiving a packet, the difference between
hop_start and hop_limit gives how many hops it traveled. */
  uint8_t hop_start;
  /* Records the public key the packet was encrypted with, if applicable. */
  meshtastic_MeshPacket_public_key_t public_key;
  /* Indicates whether the packet was en/decrypted using PKI */
  bool pki_encrypted;
  /* Last byte of the node number of the node that should be used as the next
hop in routing. Set by the firmware internally, clients are not supposed to set
this. */
  uint8_t next_hop;
  /* Last byte of the node number of the node that will relay/relayed this
packet. Set by the firmware internally, clients are not supposed to set this. */
  uint8_t relay_node;
  /* *Never* sent over the radio links.
...
as measured by the receiver. Return SNR of the last received message in dB */
  ...
  /* local channel index we heard that node on. Only populated if its not the
   * default channel. */
  uint8_t channel;
...
This is used by the phone to know how many NodeInfo packets to expect on
want_config */
...
typedef struct _meshtastic_Neighbor {
  ...
  float snr;
  /* Reception time (in secs since 1970) of last message that was last sent by
this ID. Note: this is for local storage only and will not be sent out over the
mesh. */
...
the FIFO, see our bluetooth docs */
 ...
 version of android apps. */
    ...
    /* MQTT Client Proxy Message (device sending to client / phone for
     * publishing to MQTT) */
...
/* A heartbeat message is sent to the node from the client to keep the
 connection alive. This is currently only needed to keep serial connections
 alive, but can be used by any PhoneAPI. */
...
typedef struct _meshtastic_ToRadio {
 ...
 optional for clients) */
    bool disconnect;
    meshtastic_XModem xmodemPacket;
    /* MQTT Client Proxy Message (for client / phone subscribed to MQTT sending
     * to device) */
    meshtastic_MqttClientProxyMessage mqttClientProxyMessage;
...
#define _meshtastic_CriticalErrorCode_ARRAYSIZE                                                 \
  ((meshtastic_CriticalErrorCode)(meshtastic_CriticalErrorCode_FLASH_CORRUPTION_UNRECOVERABLE + \
                                  1))
...
#define _meshtastic_FirmwareEdition_ARRAYSIZE                                  \
  ((meshtastic_FirmwareEdition)(meshtastic_FirmwareEdition_DIY_EDITION + 1))

#define _meshtastic_ExcludedModules_MIN meshtastic_ExcludedModules_EXCLUDED_NONE
#define _meshtastic_ExcludedModules_MAX                                        \
  meshtastic_ExcludedModules_NETWORK_CONFIG
#define _meshtastic_ExcludedModules_ARRAYSIZE                                  \
  ((meshtastic_ExcludedModules)(meshtastic_ExcludedModules_NETWORK_CONFIG + 1))
...
#define _meshtastic_Routing_Error_MAX                                          \
  meshtastic_Routing_Error_PKI_SEND_FAIL_PUBLIC_KEY
#define _meshtastic_Routing_Error_ARRAYSIZE                                       \
  ((meshtastic_Routing_Error)(meshtastic_Routing_Error_PKI_SEND_FAIL_PUBLIC_KEY + \
                              1))

#define _meshtastic_StoreForwardPlusPlus_SFPP_message_type_MIN                 \
  meshtastic_StoreForwardPlusPlus_SFPP_message_type_CANON_ANNOUNCE
...
#define _meshtastic_MeshPacket_TransportMechanism_MIN                          \
  meshtastic_MeshPacket_TransportMechanism_TRANSPORT_INTERNAL
#define _meshtastic_MeshPacket_TransportMechanism_MAX                          \
  meshtastic_MeshPacket_TransportMechanism_TRANSPORT_API
#define _meshtastic_MeshPacket_TransportMechanism_ARRAYSIZE                                            \
  ((meshtastic_MeshPacket_TransportMechanism)(meshtastic_MeshPacket_TransportMechanism_TRANSPORT_API + \
                                              1))
...
#define meshtastic_StoreForwardPlusPlus_sfpp_message_type_ENUMTYPE             \
  meshtastic_StoreForwardPlusPlus_SFPP_message_type
...
#define meshtastic_MeshPacket_transport_mechanism_ENUMTYPE                     \
  meshtastic_MeshPacket_TransportMechanism

#define meshtastic_MyNodeInfo_firmware_edition_ENUMTYPE                        \
  meshtastic_FirmwareEdition
...
#define meshtastic_Position_init_default                                       \
  {                                                                            \
    false, 0, false, 0, false, 0, 0, _meshtastic_Position_LocSource_MIN,       \
        _meshtastic_Position_AltSource_MIN, 0, 0, false, 0, false, 0, 0, 0, 0, \
        0, false, 0, false, 0, 0, 0, 0, 0, 0, 0, 0                             \
  }
#define meshtastic_User_init_default                                           \
  {                                                                            \
    "", "", "", {0}, _meshtastic_HardwareModel_MIN, 0,                         \
        _meshtastic_Config_DeviceConfig_Role_MIN, {0, {0}}, false, 0           \
  }
...
#define meshtastic_StoreForwardPlusPlus_init_default                           \
  {                                                                            \
    _meshtastic_StoreForwardPlusPlus_SFPP_message_type_MIN, {0, {0}},          \
...
#define meshtastic_MeshPacket_init_default                                     \
  {                                                                            \
    0, 0, 0, 0, {meshtastic_Data_init_default}, 0, 0, 0, 0, 0,                 \
        ...
        _meshtastic_MeshPacket_TransportMechanism_MIN                          \
  }
...
#define meshtastic_NeighborInfo_init_default                                   \
  {                                                                            \
    0, 0, 0, 0, {                                                              \
      meshtastic_Neighbor_init_default, meshtastic_Neighbor_init_default,      \
          meshtastic_Neighbor_init_default, meshtastic_Neighbor_init_default,  \
          meshtastic_Neighbor_init_default, meshtastic_Neighbor_init_default,  \
          meshtastic_Neighbor_init_default, meshtastic_Neighbor_init_default,  \
          meshtastic_Neighbor_init_default, meshtastic_Neighbor_init_default   \
...
#define meshtastic_DeviceMetadata_init_default                                 \
  {                                                                            \
    "", 0, 0, 0, 0, 0, _meshtastic_Config_DeviceConfig_Role_MIN, 0,            \
        _meshtastic_HardwareModel_MIN, 0, 0, 0                                 \
  }
...
#define meshtastic_NeighborInfo_init_zero                                      \
  {                                                                            \
    0, 0, 0, 0, {                                                              \
      meshtastic_Neighbor_init_zero, meshtastic_Neighbor_init_zero,            \
          meshtastic_Neighbor_init_zero, meshtastic_Neighbor_init_zero,        \
          meshtastic_Neighbor_init_zero, meshtastic_Neighbor_init_zero,        \
          meshtastic_Neighbor_init_zero, meshtastic_Neighbor_init_zero,        \
          meshtastic_Neighbor_init_zero, meshtastic_Neighbor_init_zero         \
...
#define meshtastic_MqttClientProxyMessage_FIELDLIST(X, a)                      \
  ...
  X(a, STATIC, ONEOF, STRING, (payload_variant, text, payload_variant.text),   \
    3)                                                                         \
...
#define meshtastic_FromRadio_FIELDLIST(X, a)                                   \
  ...
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, mqttClientProxyMessage, mqttClientProxyMessage), 14)     \
  X(a, STATIC, ONEOF, MESSAGE, (payload_variant, fileInfo, fileInfo), 15)      \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, clientNotification, clientNotification), 16)             \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, deviceuiConfig, deviceuiConfig), 17)
...
#define meshtastic_FromRadio_payload_variant_metadata_MSGTYPE                  \
  meshtastic_DeviceMetadata
#define meshtastic_FromRadio_payload_variant_mqttClientProxyMessage_MSGTYPE    \
  meshtastic_MqttClientProxyMessage
#define meshtastic_FromRadio_payload_variant_fileInfo_MSGTYPE                  \
  meshtastic_FileInfo
#define meshtastic_FromRadio_payload_variant_clientNotification_MSGTYPE        \
  meshtastic_ClientNotification
#define meshtastic_FromRadio_payload_variant_deviceuiConfig_MSGTYPE            \
...
#define meshtastic_ClientNotification_FIELDLIST(X, a)                          \
  ...
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, key_verification_number_inform,                          \
     payload_variant.key_verification_number_inform),                          \
    11)                                                                        \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, key_verification_number_request,                         \
     payload_variant.key_verification_number_request),                         \
    12)                                                                        \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, key_verification_final,                                  \
     payload_variant.key_verification_final),                                  \
    13)                                                                        \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, duplicated_public_key,                                   \
     payload_variant.duplicated_public_key),                                   \
    14)                                                                        \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, low_entropy_key, payload_variant.low_entropy_key), 15)
...
#define meshtastic_ClientNotification_payload_variant_key_verification_number_request_MSGTYPE \
  meshtastic_KeyVerificationNumberRequest
#define meshtastic_ClientNotification_payload_variant_key_verification_final_MSGTYPE \
  meshtastic_KeyVerificationFinal
...
#define meshtastic_ToRadio_FIELDLIST(X, a)                                     \
  ...
  X(a, STATIC, ONEOF, MESSAGE, (payload_variant, xmodemPacket, xmodemPacket),  \
    5)                                                                         \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, mqttClientProxyMessage, mqttClientProxyMessage), 6)      \
...
#define meshtastic_ToRadio_payload_variant_xmodemPacket_MSGTYPE                \
  meshtastic_XModem
#define meshtastic_ToRadio_payload_variant_mqttClientProxyMessage_MSGTYPE      \
  meshtastic_MqttClientProxyMessage
#define meshtastic_ToRadio_payload_variant_heartbeat_MSGTYPE                   \
  meshtastic_Heartbeat
...
#define meshtastic_NodeRemoteHardwarePin_pin_MSGTYPE                           \
...
#define meshtastic_ChunkedPayloadResponse_FIELDLIST(X, a)                      \
  ...
  X(a, STATIC, ONEOF, BOOL,                                                    \
    (payload_variant, request_transfer, payload_variant.request_transfer), 2)  \
  X(a, STATIC, ONEOF, BOOL,                                                    \
    (payload_variant, accept_transfer, payload_variant.accept_transfer), 3)    \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, resend_chunks, payload_variant.resend_chunks), 4)
#define meshtastic_ChunkedPayloadResponse_CALLBACK NULL
#define meshtastic_ChunkedPayloadResponse_DEFAULT NULL
#define meshtastic_ChunkedPayloadResponse_payload_variant_resend_chunks_MSGTYPE \
  meshtastic_resend_chunks
...
#define meshtastic_StoreForwardPlusPlus_fields                                 \
...
#define meshtastic_MqttClientProxyMessage_fields                               \
  &meshtastic_MqttClientProxyMessage_msg
...
#define meshtastic_KeyVerificationNumberInform_fields                          \
  &meshtastic_KeyVerificationNumberInform_msg
#define meshtastic_KeyVerificationNumberRequest_fields                         \
  &meshtastic_KeyVerificationNumberRequest_msg
#define meshtastic_KeyVerificationFinal_fields                                 \
  &meshtastic_KeyVerificationFinal_msg
#define meshtastic_DuplicatedPublicKey_fields                                  \
  &meshtastic_DuplicatedPublicKey_msg
...
#define meshtastic_NodeRemoteHardwarePin_fields                                \
...
#define meshtastic_ChunkedPayloadResponse_fields                               \
  &meshtastic_ChunkedPayloadResponse_msg
