...
"active" */
  meshtastic_ModuleConfig_DetectionSensorConfig_TriggerType_EITHER_EDGE_ACTIVE_LOW =
      4,
...
https://heltec.org/project/meshsolar/ */
  ...
  /* Logs mesh traffic to the serial pins, ideal for logging via openLog or
     similar. */
  meshtastic_ModuleConfig_SerialConfig_Serial_Mode_LOG =
      9, /* includes other packets */
  meshtastic_ModuleConfig_SerialConfig_Serial_Mode_LOGTEXT =
      10 /* only text (channel & DM) */
...
This is useful if you want to use a single MQTT server for multiple meshtastic
networks and separate them via ACLs */
  char root[32];
  /* If true, we can use the connected phone / client to proxy messages to MQTT
   * instead of a direct connection */
  bool proxy_to_client_enabled;
  /* If true, we will periodically report unencrypted information about our node
   * to a map via MQTT */
...
NeighborInfo should be transmitted over LoRa. Note that this is not available on
a channel with default key and name. */
...
module config settings This is currently only usable in output modes like NMEA /
CalTopo and may behave strangely or not work at all in other modes Existing
logging over the Serial Console will still be present */
...
typedef struct _meshtastic_ModuleConfig_StoreForwardConfig {
  ...
  /* Set to true to let this node act as a server that stores received messages
   * and resends them upon request. */
  bool is_server;
...
ESP32 Only */
  bool save;
  /* Bool indicating that the node should cleanup / destroy it's RangeTest.csv
file. ESP32 Only */
...
Note: We will still send telemtry to the connected phone / client every minute
over the API */
  bool device_telemetry_enabled;
  /* Enable/Disable the air quality telemetry measurement module on-device
   * display */
...
Can be e.g. "rotEnc1", "upDownEnc1", "scanAndSelect", "cardkb", "serialkb", or
keyword "_any" */
...
/* Ambient Lighting Module - Settings for control of onboard LEDs to allow users
to adjust the brightness levels and respective color levels. Initially created
for the RAK14001 RGB LED module. */
...
/* StatusMessage config - Allows setting a status message for a node to
...
typedef struct _meshtastic_ModuleConfig_RemoteHardwareConfig {
  /* Whether the Module is enabled */
  bool enabled;
  /* Whether the Module allows consumers to read / write to pins not defined in
   * available_pins */
...
#define _meshtastic_RemoteHardwarePinType_MIN                                  \
  meshtastic_RemoteHardwarePinType_UNKNOWN
#define _meshtastic_RemoteHardwarePinType_MAX                                  \
  meshtastic_RemoteHardwarePinType_DIGITAL_WRITE
#define _meshtastic_RemoteHardwarePinType_ARRAYSIZE                                    \
  ((meshtastic_RemoteHardwarePinType)(meshtastic_RemoteHardwarePinType_DIGITAL_WRITE + \
                                      1))
...
#define _meshtastic_ModuleConfig_SerialConfig_Serial_Mode_ARRAYSIZE                                              \
  ((meshtastic_ModuleConfig_SerialConfig_Serial_Mode)(meshtastic_ModuleConfig_SerialConfig_Serial_Mode_LOGTEXT + \
                                                      1))
...
#define meshtastic_RemoteHardwarePin_type_ENUMTYPE                             \
  meshtastic_RemoteHardwarePinType
...
#define meshtastic_ModuleConfig_MQTTConfig_init_default                        \
  {                                                                            \
    0, "", "", "", 0, 0, 0, "", 0, 0, false,                                   \
        meshtastic_ModuleConfig_MapReportSettings_init_default                 \
  }
...
#define meshtastic_ModuleConfig_RemoteHardwareConfig_init_default              \
  {                                                                            \
    0, 0, 0, {                                                                 \
      meshtastic_RemoteHardwarePin_init_default,                               \
          meshtastic_RemoteHardwarePin_init_default,                           \
          meshtastic_RemoteHardwarePin_init_default,                           \
          meshtastic_RemoteHardwarePin_init_default                            \
    }                                                                          \
...
#define meshtastic_ModuleConfig_DetectionSensorConfig_init_default             \
  {                                                                            \
    0, 0, 0, 0, "", 0,                                                         \
        _meshtastic_ModuleConfig_DetectionSensorConfig_TriggerType_MIN, 0      \
  }
...
#define meshtastic_ModuleConfig_MQTTConfig_init_zero                           \
  {                                                                            \
    0, "", "", "", 0, 0, 0, "", 0, 0, false,                                   \
        meshtastic_ModuleConfig_MapReportSettings_init_zero                    \
  }
...
#define meshtastic_ModuleConfig_DetectionSensorConfig_minimum_broadcast_secs_tag \
  2
...
#define meshtastic_ModuleConfig_DetectionSensorConfig_detection_trigger_type_tag \
  7
...
#define meshtastic_ModuleConfig_PaxcounterConfig_paxcounter_update_interval_tag \
  2
...
#define meshtastic_ModuleConfig_ExternalNotificationConfig_use_i2s_as_buzzer_tag \
  15
...
#define meshtastic_ModuleConfig_TelemetryConfig_health_measurement_enabled_tag \
  11
...
#define meshtastic_ModuleConfig_TelemetryConfig_air_quality_screen_enabled_tag \
  15
...
#define meshtastic_ModuleConfig_RemoteHardwareConfig_allow_undefined_pin_access_tag \
  2
...
#define meshtastic_ModuleConfig_FIELDLIST(X, a)                                \
  ...
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, neighbor_info, payload_variant.neighbor_info), 10)       \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, ambient_lighting, payload_variant.ambient_lighting), 11) \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, detection_sensor, payload_variant.detection_sensor), 12) \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, paxcounter, payload_variant.paxcounter), 13)             \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, statusmessage, payload_variant.statusmessage), 14)
...
#define meshtastic_ModuleConfig_payload_variant_remote_hardware_MSGTYPE        \
  meshtastic_ModuleConfig_RemoteHardwareConfig
#define meshtastic_ModuleConfig_payload_variant_neighbor_info_MSGTYPE          \
  meshtastic_ModuleConfig_NeighborInfoConfig
#define meshtastic_ModuleConfig_payload_variant_ambient_lighting_MSGTYPE       \
  meshtastic_ModuleConfig_AmbientLightingConfig
#define meshtastic_ModuleConfig_payload_variant_detection_sensor_MSGTYPE       \
  meshtastic_ModuleConfig_DetectionSensorConfig
#define meshtastic_ModuleConfig_payload_variant_paxcounter_MSGTYPE             \
  meshtastic_ModuleConfig_PaxcounterConfig
#define meshtastic_ModuleConfig_payload_variant_statusmessage_MSGTYPE          \
  meshtastic_ModuleConfig_StatusMessageConfig
...
#define meshtastic_ModuleConfig_MQTTConfig_map_report_settings_MSGTYPE         \
  meshtastic_ModuleConfig_MapReportSettings
...
#define meshtastic_ModuleConfig_RemoteHardwareConfig_available_pins_MSGTYPE    \
  meshtastic_RemoteHardwarePin
...
#define meshtastic_ModuleConfig_MapReportSettings_fields                       \
...
#define meshtastic_ModuleConfig_NeighborInfoConfig_fields                      \
  &meshtastic_ModuleConfig_NeighborInfoConfig_msg
#define meshtastic_ModuleConfig_DetectionSensorConfig_fields                   \
  &meshtastic_ModuleConfig_DetectionSensorConfig_msg
#define meshtastic_ModuleConfig_AudioConfig_fields                             \
  &meshtastic_ModuleConfig_AudioConfig_msg
#define meshtastic_ModuleConfig_PaxcounterConfig_fields                        \
  &meshtastic_ModuleConfig_PaxcounterConfig_msg
...
#define meshtastic_ModuleConfig_AmbientLightingConfig_fields                   \
  &meshtastic_ModuleConfig_AmbientLightingConfig_msg
#define meshtastic_ModuleConfig_StatusMessageConfig_fields                     \
  &meshtastic_ModuleConfig_StatusMessageConfig_msg
#define meshtastic_RemoteHardwarePin_fields &meshtastic_RemoteHardwarePin_msg

/* Maximum encoded size of messages (where known) */
#define MESHTASTIC_MESHTASTIC_MODULE_CONFIG_PB_H_MAX_SIZE                      \
