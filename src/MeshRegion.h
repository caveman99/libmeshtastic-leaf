...
constexpr int16_t OVERRIDE_SLOT_CHANNEL_HASH = 0;
constexpr int16_t OVERRIDE_SLOT_PRESET_HASH = -1;

// Regulatory parameters shared by several regions. spacing is the gap between
// operating channels and at the start of the band; padding is the gap at each
// side of a channel, which is how a narrow bandwidth is coerced up to a wider
// regulatory channel.
struct RegionProfile {
  float spacing;       ///< MHz
  float padding;       ///< MHz, applied at both sides
  bool audioPermitted; ///< voice traffic allowed
...
struct RegionInfo {
  ...
  bool audioPermitted() const { return profile->audioPermitted; }
...
static const RegionProfile PROFILE_STD = {0.0f, 0.0f, true, false};
static const RegionProfile PROFILE_EU868 = {0.0f, 0.0f, false, false};
static const RegionProfile PROFILE_UNDEF = {0.0f, 0.0f, true, false};
static const RegionProfile PROFILE_LITE = {0.4f, 0.0375f, false, false};
static const RegionProfile PROFILE_NARROW = {0.0f, 0.0104f, true, false};
// 15.6 kHz bandwidth coerced up to a 20 kHz channel by padding.
static const RegionProfile PROFILE_HAM_20KHZ = {0.0f, 0.0022f, false, true};
// 62.5 kHz bandwidth coerced up to a 100 kHz channel by padding.
static const RegionProfile PROFILE_HAM_100KHZ = {0.0f, 0.01875f, false, true};
...
inline uint32_t MeshRegion::getDefaultSlot(RegionCode code, ModemPreset preset,
  ...
  // the firmware's does.
  const char *name = (channelName != nullptr && channelName[0] != ' ')
                         ? channelName
