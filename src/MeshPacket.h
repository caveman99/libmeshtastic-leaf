...
public:
   ...
   * @param buffer Output buffer (must be at least MESHTASTIC_HEADER_LENGTH
   * bytes)
   */
   ...
   *
   * @param buffer Input buffer (must be at least MESHTASTIC_HEADER_LENGTH
   * bytes)
   ...
   *
   * @param header Packet header (must have to, from, id set; channel will be
   * set to 0)
   * @param payload Plaintext payload
   * @param payloadLen Payload length
   ...
   *
   * Creates a pseudo-random packet ID using the node number as a seed
   * component.
   *
