...
void MeshPacketCodec::unpackHeader(const uint8_t* buffer, PacketHeader& header) {
    // Unpack header from little-endian format

    // to (4 bytes, little-endian)
    header.to = ((uint32_t)buffer[0] << 0) |
                ((uint32_t)buffer[1] << 8) |
                ((uint32_t)buffer[2] << 16) |
                ((uint32_t)buffer[3] << 24);

    // from (4 bytes, little-endian)
    header.from = ((uint32_t)buffer[4] << 0) |
                  ((uint32_t)buffer[5] << 8) |
...
bool MeshPacketCodec::encodePacketPKI(PacketHeader& header,
    ...
    if (!pki.encrypt(header.to, header.from, remotePubKey, header.id,
                     payload, payloadLen, payloadDst)) {
        return false;
    }
...
ReceiveResult MeshPacketCodec::decodePacket(const uint8_t* buffer, size_t bufLen,
                                            MeshChannel& channel,
...
ReceiveResult MeshPacketCodec::decodePacketPKI(const uint8_t* buffer, size_t bufLen,
                                               MeshCryptoPKI& pki,
...
bool MeshPacketCodec::encodeDataMessage(meshtastic_PortNum portNum,
    ...
    // Encode with nanopb
    pb_ostream_t stream = pb_ostream_from_buffer(outBuffer, MAX_ENCRYPTED_PAYLOAD);
