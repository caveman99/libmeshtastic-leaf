...
// RegionCode and ModemPreset from upstream config.proto, native so no public
// header pulls in config.pb.h. Append only, never renumber.
...
/// The mesh keeps the lowest node numbers for itself.
constexpr uint32_t NUM_RESERVED_NODE_NUMS = 4;

/// Packet ids carry a rolling counter in the low bits, random above it.
constexpr uint8_t ID_COUNTER_BITS = 10;
constexpr uint32_t ID_COUNTER_MASK = (1UL << ID_COUNTER_BITS) - 1;

constexpr uint8_t MESHTASTIC_SYNC_WORD = 0x2B;

constexpr uint16_t DEFAULT_PREAMBLE_LENGTH = 16;
constexpr uint16_t WIDE_LORA_PREAMBLE_LENGTH = 12;

/// Time a node needs to receive a packet and turn a reply around, empirical.
constexpr uint32_t PROCESSING_TIME_MSEC = 4500;

/// Firmware default for announcing a node's name and key.
constexpr uint32_t DEFAULT_NODE_INFO_SECS = 3 * 60 * 60;

/// The firmware refuses to announce more often than this.
constexpr uint32_t MIN_NODE_INFO_SECS = 60 * 60;
...
enum class SendResult {
  ...
  NO_IDENTITY,  ///< No public key set, so there is no node number
  INVALID_PARAM,
  TOO_LONG,
  NO_CHANNEL,
  RADIO_ERROR
...
struct MeshPacket {
  ...
  bool hasSignature;         ///< True if signature holds an XEdDSA signature
  uint8_t signature[64];     ///< Unverified; see MeshCryptoPKI::verifyPayload
