...
constexpr size_t MAX_ENCRYPTED_PAYLOAD = MAX_LORA_PAYLOAD_LEN - MESHTASTIC_HEADER_LENGTH;

/// Maximum plaintext payload for PKI messages
constexpr size_t MAX_PKI_PAYLOAD = MAX_ENCRYPTED_PAYLOAD - MESHTASTIC_PKC_OVERHEAD;
...
// Use libmeshtastic_leaf::ModemPreset (alias for meshtastic_Config_LoRaConfig_ModemPreset)
...
struct __attribute__((packed)) PacketHeader {
    ...
    uint8_t getHopLimit() const {
        return flags & PACKET_FLAGS_HOP_LIMIT_MASK;
    }

    /// Set hop limit in flags
    void setHopLimit(uint8_t limit) {
        flags = (flags & ~PACKET_FLAGS_HOP_LIMIT_MASK) | (limit & PACKET_FLAGS_HOP_LIMIT_MASK);
    }

    /// Get hop start from flags
    uint8_t getHopStart() const {
        return (flags & PACKET_FLAGS_HOP_START_MASK) >> PACKET_FLAGS_HOP_START_SHIFT;
    }

    /// Set hop start in flags
    void setHopStart(uint8_t start) {
        flags = (flags & ~PACKET_FLAGS_HOP_START_MASK) |
                ((start << PACKET_FLAGS_HOP_START_SHIFT) & PACKET_FLAGS_HOP_START_MASK);
    }

    /// Check if want_ack flag is set
    bool wantAck() const {
        return (flags & PACKET_FLAGS_WANT_ACK_MASK) != 0;
    }
    ...
    bool viaMqtt() const {
        return (flags & PACKET_FLAGS_VIA_MQTT_MASK) != 0;
    }

    /// Check if this is a PKI-encrypted packet
    bool isPKI() const {
        return channel == 0 && to != BROADCAST_ADDR;
    }
};
...
struct MeshPacket {
    ...
    MeshPacket() : portNum(meshtastic_PortNum_UNKNOWN_APP),
                   payloadLen(0), rxRssi(0), rxSnr(0), rxTime(0),
                   isPKI(false), channelIndex(0) {
...
struct RadioConfig {
    ...
    RadioConfig() : type(RadioType::SX1262),
                    region(meshtastic_Config_LoRaConfig_RegionCode_US),
                    frequency(0.0f), txPower(0),
