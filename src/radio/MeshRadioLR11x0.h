 ...
 */
template <typename T> class MeshRadioLR11x0 : public MeshRadio {
public:
  /**
  ...
  bool begin(const RadioConfig &config) override {
    ...
    int state = module_->begin(config.frequency, params.bw, params.sf,
                               params.cr, MESHTASTIC_SYNC_WORD, config.txPower,
                               DEFAULT_PREAMBLE_LENGTH, config.tcxoVoltage);
