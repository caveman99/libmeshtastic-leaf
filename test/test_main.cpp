...
TEST(packet_header_size) {
    ASSERT_EQ(sizeof(PacketHeader), 16);
}
...
TEST(modem_preset_wide_lora) {
    ModemParams params_normal = MeshRegion::getModemParams(PRESET_LONG_FAST, false);
...
TEST(preset_name) {
    ...
    ASSERT_STREQ(MeshRegion::getPresetName(PRESET_VERY_LONG_SLOW), "Very Long Slow");
}
